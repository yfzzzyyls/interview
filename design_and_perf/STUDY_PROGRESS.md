...
## Day 10: SRAM Controller + SPI Quiz (2026-04-10)

### Modules Completed
- `sram_1r1w.sv` — 1R1W behavioral SRAM macro model
- `sram_controller.sv` — banked SRAM controller with byte strobes, RMW, 2-state FSM
- `tb_sram_controller.sv` — 6 tests pass (full write/read, bank isolation, byte strobes, multi-addr)
- SPI slave (`spi_sub.sv`) — previously completed in `/home/fy2243/soc_design/labs/lab2/`

### SRAM Controller Quiz (Score: 6/8)

**Q93. In a 1R1W SRAM-based controller, why does a partial write require RMW but a full write doesn't?**
- ✅ Partial (0.5/1)
- My answer: Partial write requires reading the 32-bit value first.
- Complete answer: SRAM only supports **full-word writes**. It can't write just byte 0. So you read the old 32 bits, replace the target bytes using byte strobes, write back the full word. Full write doesn't need this because all bytes are overwritten — no old data needed.

**Q94. Walk through the exact operations for a partial write in each FSM state (IDLE, RESP).**
- ✅ Good (1/1)
- IDLE: accept the write request, fire `rd_cs` to read the old 32-bit word from the selected bank. Transition to RESP.
- RESP: `sram_rdata` now valid. Merge old data with new data using byte strobes. Fire `wr_cs` with merged data. Pulse `resp_valid`.

**Q95. Why does the controller always fire `rd_cs` in IDLE even for a full write?**
- ✅ Partial (0.5/1)
- My answer: More efficient implementation since read with resp_valid=0 won't hurt.
- Complete answer: The read is **needed** for CPU reads and partial writes. Only full writes don't need it. Gating it off adds mux logic on a timing-sensitive path (SRAM chip-select setup) for marginal power savings. Simpler to always read — it's harmless because 1R1W has independent read and write ports.

**Q96. What is `merged_data` and how is it computed? Write the merge logic.**
- ❌ Partial (0.5/1)
- My answer: Using write strobe to only write part of the 32-bit value.
- Correct concept but must know the code:
```systemverilog
merged_data = sram_rdata[bank_sel];          // start with old word
for (int b = 0; b < WIDTH/8; b++)
    if (lat_wstrb[b])
        merged_data[b*8 +: 8] = lat_wdata[b*8 +: 8];  // overwrite strobed bytes
```

**Q97. Why latch `req_we`, `req_addr`, `req_wdata`, `req_wstrb` into `lat_*` registers?**
- ✅ Good (1/1)
- CPU won't hold signals for multiple cycles but SRAM read/write takes multiple cycles, so the controller needs to remember the operation during the entire process.

**Q98. `sram_rdata[bank_sel]` — what does this give you?**
- ✅ Good (1/1)
- The requested 32-bit data word from the selected bank's read port, not the entire bank contents. `sram_rdata` is an unpacked array of NUM_BANKS entries, each WIDTH bits wide.

**Q99. The partial write path in RESP has a long combinational path. Name two ways to fix timing violations.**
- ✅ Good (1/1)
- Split into more pipeline stages (READ → MERGE → WRITE). Logic retiming.
- Bonus answer: Use SRAM macro with native `wmask` (byte write mask) — eliminates RMW entirely.

**Q100. How does `resp_valid` stay high for exactly 1 cycle?**
- ✅ Partial (0.5/1)
- My answer: When state register stores RESP, move to IDLE next cycle.
- Complete answer — the **mechanism** is the default assignment pattern:
```systemverilog
resp_valid <= 1'b0;           // default: always 0
if (state == RESP)
    resp_valid <= 1'b1;       // override: 1 for this one cycle only
```
Last nonblocking assignment wins. RESP lasts 1 cycle → pulse lasts 1 cycle.

### SPI Quiz (Score: 1.5/6)

**Q101. What are the 4 SPI wires and what does each do?**
- ✅ Partial (0.5/1)
- My answer: SCLK (main clk), CS_N (chip enable), MOSI (master out slave in), MISO (master in slave out). "Why CS_N not CS?"
- Corrections: SCLK is a dedicated SPI clock generated by the master, not the system clock. CS_N is active low (`_N` = negated) — historical TTL convention, idle state is high (pull-up), unconnected slaves stay deselected.

**Q102. Fundamental difference between SPI and UART? Why is SPI faster?**
- ✅ Partial (0.5/1)
- My answer: SPI uses master's clock synchronously. Faster because it avoids baud rate overhead.
- Complete answer: SPI is faster because (1) no start/stop bits (UART wastes 2/10 = 20% overhead), (2) no 16x oversampling needed, (3) explicit clock eliminates drift tolerance. Not "avoids baud rate" — SPI still has a data rate.

**Q103. CPOL=0, CPHA=0 (Mode 0): when is data sampled vs shifted?**
- ❌ (0/1)
- Answer: CPOL=0 = SCLK idles low. CPHA=0 = data **sampled on rising edge** (leading), **shifted on falling edge** (trailing). Mode 0 is most common. In `spi_sub.sv` — posedge block handles RX (sample), negedge block handles TX (shift).

**Q104. SPI is full-duplex. What does that mean per clock edge?**
- ❌ Partial (0.5/1)
- My answer: Bidirectional data transfer.
- Complete: On every clock edge, master sends 1 bit on MOSI **and** slave sends 1 bit on MISO **simultaneously**. Two shift registers connected in a ring. Full-duplex = both directions at once. Half-duplex = one at a time (like walkie-talkie).

**Q105. SPI slave runs on SCLK but internal logic runs on system clock. Problem and solution?**
- ❌ (0/1)
- Answer: **Clock domain crossing** problem. Solutions: 2-FF synchronizer, toggle handshake (used in `spi_sub.sv` as `resp_tog`/`resp_tog_q`), or async FIFO between SCLK and system clock domains.

**Q106. Why doesn't SPI need start/stop bits or 16x oversampling?**
- ❌ (0/1)
- UART needs start bit (detect frame start when no clock), stop bit (mark frame end), and 16x oversampling (sample mid-bit to tolerate clock drift).
- SPI has explicit SCLK — tells slave exactly when to sample. No guessing, no drift, no frame detection needed. CS_N defines the frame boundary instead of start/stop bits.

### Key Concepts Learned

**16x Oversampling (UART RX):**
- Receiver runs counter at 16× baud rate, each bit divided into 16 ticks
- Sample at tick 8 (mid-bit) for maximum noise margin
- Start bit detection: line drops low → wait 8 ticks → confirm still low → aligned to mid-bit
- From there, sample every 16 ticks → always hitting center of each bit
- Tolerates sender/receiver clock drift of several percent

**UART vs SPI coexistence:**
- UART: few wires (2), long distance (meters), slow (~115kbps). Use: debug console, GPS, Bluetooth, inter-board
- SPI: more wires (4+), short distance (on-board cm), fast (~50MHz+). Use: flash, sensors, ADC, displays
- Can't use SPI over long distance (SCLK skew), can't use UART for fast peripherals (too slow)

### Credit-Based Flow Control

**Modules Completed:**
- `credit_sender.sv` — credit counter + data passthrough, no buffer
- `credit_receiver.sv` — FIFO buffer + credit return pulse on pop
- `tb_credit_flow.sv` — 4 tests pass (exhaust credits, drain buffer, credit replenish, streaming with slow consumer)

### Credit-Based Flow Quiz (Score: 4/6)

**Q107. What problem does credit-based flow solve that valid/ready can't handle well?**
- ✅ Good (1/1)
- Long combinational backpressure chain — `ready` signal must traverse the full pipeline path, creating a long combinational path that kills timing.

**Q108. How does the sender know whether the receiver has buffer space, without a `ready` signal?**
- ❌ Partial (0.5/1)
- My answer: Sender gets update of the number of buffer entries every now and then.
- Complete answer: Sender tracks a **local credit counter**. Initialized to `MAX_CREDITS` (= buffer depth) at reset. Decrements on send, increments on `credit_return` pulse from receiver. The counter always reflects available buffer space. Key difference from valid/ready: the `credit_return` pulse can arrive **late** (pipelined) and the system still works because the buffer absorbs the delay.

**Q109. Why is there no FIFO in the sender? Where does buffering happen and why?**
- ✅ Partial (0.5/1)
- My answer: Backpressure is from receiver to sender, so FIFO is in the receiver.
- Complete answer: Sender is a **passthrough** — gates data with credit count, no storage. Receiver has the FIFO to **absorb flits in flight** during pipeline delay. Buffer size matches credits to mathematically guarantee no overflow: `credits_at_sender + flits_in_pipeline + entries_in_buffer = MAX_CREDITS` (always).

**Q110. Pipeline is 4 cycles each direction. Minimum buffer depth for full throughput?**
- ❌ Wrong (0/1)
- My answer: 5 (4 + 1).
- Correct: Round trip = 4 + 4 = **8 cycles**. Minimum buffer = 8 + 1 = **9**. The +1 covers the cycle where the pop happens and the credit return starts its journey back. With 9 credits, sender fires for 9 cycles; first credit return arrives at cycle 8 — just in time, no bubble.

**Q111. `src_ready = (credit_count > 0)` is combinational. Doesn't that defeat the purpose?**
- ✅ Good (1/1)
- No — `src_ready` is driven from a **local register** (credit_count), not from the far-away receiver. The combinational path is short (sender ↔ upstream source, same block). Credit-based flow keeps valid/ready local and replaces the long-distance backpressure with credit counting.

**Q112. What happens when sender sends a flit and receives a credit return in the same cycle?**
- ✅ Good (1/1)
- They cancel out, credit count stays the same. The `default` case in `case({send, credit_return})` handles both `2'b00` and `2'b11`.

### Key Concepts: Credit-Based Flow

**Credit invariant:** `credits_at_sender + flits_in_pipeline + entries_in_buffer = MAX_CREDITS` (always holds)

**Sizing rule:** `buffer_depth >= round_trip_latency + 1` (round trip = both directions!)

**When to use:**
- Valid/ready: short distance (same block, 1-2 hops), simple interfaces (AXI, APB)
- Credit-based: long distance (NoC, cross-chip, PCIe), pipeline depth > 1 cycle between sender and receiver

**Optimization:** batch credit returns (return multi-bit count instead of 1-bit pulse per pop) to reduce toggle rate on return wire
